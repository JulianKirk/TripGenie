...
def allow_query_params(*allowed: str):
    ...
    def dependency(request: Request) -> None:
        extras = sorted(set(request.query_params) - allowed_set)
        if not extras:
            return
...
def _parse_enum_filter(value: str | None, *, field: str, enum_type, allowed: str):
    ...
    try:
        ...
        raise validation_error(
            VALIDATION_ERROR_MESSAGE,
            [{"field": field, "issue": f"must be one of: {allowed}"}],
        ) from exc
...
def parse_compare_ids(
    ...
    if not value:
        _raise_query_error("ids", "must select at least one transport option")
    ...
    for raw in value or []:
        for part in raw.split(","):
            ...
            if not candidate:
                continue
            if TRANSPORT_ID_PATTERN.fullmatch(candidate) is None:
                _raise_query_error(
                    "ids",
                    f"must match {TRANSPORT_ID_PATTERN.pattern}",
                )
...
ServiceDep = Annotated[BackendService, Depends(get_service)]
TransportTypeFilter = Annotated[
    TransportType | None,
...
def create_app(
    ...
    *,
    ...
    def ready(response: Response, service: ServiceDep) -> dict[str, object]:
    ...
    @router.get(
        "/transport-options/compare",
        dependencies=[compare_query_params],
        response_model=DataEnvelope[list[TransportOptionRecord]],
    )
    ...
    @router.get(
        ...
        dependencies=[no_query_params],
        response_model=DataEnvelope[TripTransportSummary],
    )
    def trip_transport(
