...
def clear_ai_mode_env(monkeypatch: pytest.MonkeyPatch) -> None:
    for env_name in (
        ...
        "AI_MODE_DEFAULT_EMBEDDING_MODEL",
        ...
        "AI_MODE_MAX_EMBED_INPUTS",
        "AI_MODE_MAX_EMBED_INPUT_CHARS",
        "AI_MODE_MAX_EMBED_DIMENSIONS",
    ):
...
def test_settings_default_to_host_ollama_loopback(
    ...
    assert settings.default_embedding_model == "nomic-embed-text"
