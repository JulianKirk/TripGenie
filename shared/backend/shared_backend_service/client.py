...
class DatabaseClient:
    ...
    async def get(self, resource: str, place_id: UUID) -> dict[str, Any]:
        """`resource` is "country" or "city" -- the two endpoints differ only
        by that word, so one method serves both."""
        return await self.request("GET", f"{PATH}/{resource}/{place_id}")
...
async def request(
    ...
    **kwargs: Any,
    ...
    Module-level so every upstream this service ever calls fails the same
    documented way. There is only ever one copy of this mapping.
    """
    try:
