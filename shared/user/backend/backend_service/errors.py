"""Error handling for the user backend service.

The API doc specifies 400 for "invalid input / validation error"; FastAPI's
default for a schema violation is 422. This remaps it so callers see the
documented status, keeping FastAPI's `{"detail": [...]}` body -- the doc lists
status codes only, so there is no error envelope to match.

The 409 on a taken username is not handled here: it is the database service's
answer, and `client.request` passes a 4xx through unchanged.
"""
