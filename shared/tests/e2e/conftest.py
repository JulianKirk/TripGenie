"""Fixtures for the shared reference end-to-end tests.
...
end-to-end: the two services declare the country and city messages separately
(see shared_backend_service/schemas.py), so any drift between the two fails
...
from shared_database_service.app import create_app as create_database_app
from shared_database_service.config import Settings as DatabaseSettings
...
def database(tmp_path):
    ...
    app = create_database_app(
        DatabaseSettings(
            database_url=f"sqlite:///{tmp_path / 'location.db'}", seed=False
        )
    )
