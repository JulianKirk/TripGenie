...
class TestAccommodationRepository:
    ...
    def test_search_by_min_room_count(self, accommodations, camping, hotel):
        """SQL-level filter via the RoomDetails join."""
        rows, total = accommodations.search(min_room_count=10)
        assert [a.name for a in rows] == ["Grand Hotel"]
        assert total == 1

    def test_search_by_city(self, accommodations, camping, hotel):
        rows, _ = accommodations.search(country="Australia", city="Sydney")
        assert [a.name for a in rows] == ["Grand Hotel"]
    ...
    def test_search_filters_stack(self, accommodations, camping, hotel):
        ...
        rows, total = accommodations.search(
            country="Australia", city="Sydney", min_room_count=10
        )
        assert [a.name for a in rows] == ["Grand Hotel"]
        assert total == 1

        rows, total = accommodations.search(
            country="Australia", city="Katoomba", min_room_count=10
        )
        assert rows == []
        assert total == 0
    ...
    def test_search_paginates_but_totals_the_whole_match(
        ...
        rows, total = accommodations.search(limit=1)
        assert len(rows) == 1
        assert total == 2
